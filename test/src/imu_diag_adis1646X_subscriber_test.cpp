 ...
 * values.
 */
TEST(ImuDiagSubscriberTest, test_imu_diag_data_publisher_adis1646x)
{
  ...
  if (device_descriptor->getDeviceFamily() != "adis1646x") {
  ...
  auto callback = [&callbackExecuted,
                   &device_descriptor](adi_imu::msg::ImuDiagDataADIS1646X msg) -> void {
  ...
  auto subscriber = node->create_subscription<adi_imu::msg::ImuDiagDataADIS1646X>(topic, 10, callback);
