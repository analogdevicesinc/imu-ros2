 ...
 * values.
 */
TEST(ImuDiagSubscriberTest, test_imu_diag_data_publisher_adis1654x)
{
  ...
  if (device_descriptor->getDeviceFamily() != "adis1654x") {
  ...
  auto callback = [&callbackExecuted,
                   &device_descriptor](adi_imu::msg::ImuDiagDataADIS1654X msg) -> void {
  ...
  auto subscriber = node->create_subscription<adi_imu::msg::ImuDiagDataADIS1654X>(topic, 10, callback);
