 ...
 * values.
 */
TEST(ImuDiagSubscriberTest, test_imu_diag_data_publisher_adis1657x)
{
  ...
  if (device_descriptor->getDeviceFamily() != "adis1657x") {
  ...
  auto callback = [&callbackExecuted,
                   &device_descriptor](adi_imu::msg::ImuDiagDataADIS1657X msg) -> void {
  ...
  auto subscriber = node->create_subscription<adi_imu::msg::ImuDiagDataADIS1657X>(topic, 10, callback);
